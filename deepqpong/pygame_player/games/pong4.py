...
class Menu():
    ...
    def __init__(self, pos, data=[], textsize=32,
        ...
        for word in self.data:
            self.textsurfs.append(self.font.render(
                str(word), True, self.wordcolor))
    ...
    def add(self, words):
        ...
        for word in words:
            self.textrects.append(self.font.render(
                str(word), True, self.wordcolor).get_rect())
            self.textsurfs.append(self.font.render(
                str(word), True, self.wordcolor))
    ...
    def update(self, screen, event):
        ...
        for n in self.pos:
            curpos.append(n)
        curpos[0] += 1
        # Define the max height of the text box
        self.rect.height = len(self.data) * self.textsize
        if self.rect.height > screen.get_height():
            self.scroll = True
        else:
            self.scroll = False
        # Define the max width of the text box based on the word length it
        # holds
        maxsize = sorted(self.data)
        maxsize = len(maxsize[0])
        self.rect.width = maxsize * self.textsize / 2
        pygame.draw.rect(screen, self.backcolor,
                         self.rect)  # Draw the text box
        pygame.draw.rect(screen, self.wordcolor, self.rect,
                         1)  # Draw the text box outline
        ...
        for surf in self.textsurfs:
            if not self.textsurfs.index(surf) == self.cursorpos:
                ...
                surf = self.font.render(
                    str(self.data[self.textsurfs.index(surf)]), True, self.selectedcolor)
                screen.blit(surf, curpos)
...
def menu():
    ...
    titlemenu = Menu([20, 65], wordcolor=[255, 0, 0], selectedcolor=[
                     255, 255, 0], data=['1-Player game', '2-Player game', 'Quit'])
    screen.fill(black)
    font = pygame.font.Font(None, 32)
    screen.blit(font.render(
        'The controls are: Up-Down for Player 1', True, white, black), [0, 0])
    screen.blit(font.render('W-S for Player 2. Solo: Up-Down',
    ...
    class Ball():
        ...
        def update(self, paddle, enemy):
            ...
            if self.rect.colliderect(
                    paddle.rect) or self.rect.colliderect(enemy.rect):
                self.paddlecols += 1
...
def gameLoop(paddle, enemy, ball, twoplayer, paddle_two):
    ...
    while not topscore > 9:
        ...
        topscore = topscore[0]
        screen.blit(paddlescore.render(
            str(paddle.score), True, white), [270, 0])
        if twoplayer:
            screen.blit(enemyscore.render(
                str(paddle_two.score), True, white), [360, 0])
        else:
            screen.blit(enemyscore.render(
                str(enemy.score), True, white), [360, 0])
    ...
    if not twoplayer:
        if paddle.score == topscore:
            screen.blit(paddlescore.render(
                'You Won! You beat the machine, proving that YOU', True, white, black), [0, 0])
            screen.blit(paddlescore.render(
                'are the Master Pong Player! Well, not really.', True, white, black), [0, 32])
            screen.blit(paddlescore.render(
                'But it was a nice thought. (Press any key to', True, white, black), [0, 64])
            screen.blit(paddlescore.render('return to the menu).',
                                           True, white, black), [0, 98])
            pygame.display.flip()
            while True:
                for event in pygame.event.get():
            ...
            screen.blit(paddlescore.render(
                'You Lost! This bucket of bolts beat a smart human like', True, white, black), [0, 0])
            screen.blit(paddlescore.render(
                'yourself! Too bad! Press any key to return to menu.', True, white, black), [0, 32])
            pygame.display.flip()
            while True:
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN:
        ...
        if paddle.score == topscore:
            screen.blit(paddlescore.render(
                'Player 1 won! (left side). Press any key to', True, white, black), [0, 0])
            screen.blit(paddlescore.render(
                'to menu.', True, white, black), [0, 32])
            pygame.display.flip()
            while True:
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN:
            ...
            screen.blit(paddlescore.render(
                'Player 2 won! (right side). Press any key to', True, white, black), [0, 0])
            screen.blit(paddlescore.render(
                'to menu.', True, white, black), [0, 32])
            pygame.display.flip()
            while True:
